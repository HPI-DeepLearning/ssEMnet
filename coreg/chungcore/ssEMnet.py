...
class ssEMnet(object):
    ...
    def getssEMnet(self):
        ...
        x = SpatialTransformer(localization_net=self.locnet, output_size=self.input1_shape, input_shape=self.input1_shape, name='stm')(input1)
        g = ConvAutoEncoder2D(self.ModelFileAutoEncoder, x.get_shape().as_list()[0], x.get_shape().as_list()[1])
        x1 = g.encode(x,1)
        ...
        autoencoder = Model(input1, g.decode(g.encode(input1,1))) # encode layers start naming at 1 and end at 6 
        ...
        for l in model.layers:
            if 'encode' in l.name:
                ...
                weights = autoencoder.get_layer('encode'+str((i+5)%6+1)).get_weights()
                l.set_weights(weights)
    ...
    def train(self, X1, X2):
        ...
        model.fit([X1,X2],np.zeros((X1.shape[0],)),batch_size=1, epochs=10, shuffle=True, verbose=1, validation_split=0.2, callbacks=[model_checkpoint])
    ...
    def predictModel(self, X1, imageSink): 
        ...
        transformed_images = transformed_images / np.amax(abs(transformed_images))
